...
async def create_acc(request: Request, data: SignUpRequest) -> dict:
    ...
    try:
        res = supabase.auth.sign_up({
            ...
            "options": {
                "data": {"name": name}, 
                "email_redirect_to": f"{FRONTEND_URL}/auth/email-confirmed"
                }
            })
    ...
    return {
        ...
        **_provision_confirmed_user(access_token=res.session.access_token)
        }
...
async def resend_confirmation(request: Request, data: dict = Body(...)) -> dict:
    ...
    try:
        supabase.auth.resend({
            "type": "signup", 
            "email": email, 
            "options": {"email_redirect_to": f"{FRONTEND_URL}/auth/email-confirmed"}
            })
    except AuthApiError:
        raise HTTPException(
            status_code=400, 
            detail="Unable to resend confirmation email. Please check the email address and try again."
            )
    except Exception:
        traceback.print_exc()
        raise HTTPException(
            status_code=500, 
            detail="Unable to resend confirmation email"
            )
...
async def sign_in(request: Request, data: SignInRequest) -> dict:
    ...
    try:
        ...
        if "email not confirmed" in msg or "email_not_confirmed" in msg:
            raise HTTPException(
                status_code=403, 
                detail="Please confirm your email address before signing in. Check your inbox or resend the confirmation email."
                )
        raise HTTPException(
            status_code=401, 
        ...
        raise HTTPException(
            status_code=500, 
            detail="Unexpected sign-in error"
            )
