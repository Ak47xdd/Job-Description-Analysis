...
Currently Supports:
    ...
    - API Authenticator (header: JobAnalyze_6k_Key)
    - API Encryptor (SHA-256)
    - JobAnalyze 6k model Endpoint
    - Auth: Sign Up / Sign In via Supabase
    - MCP Server (key-protected, same header)
"""
...
if __name__ == "__main__":
    uvicorn.run("JobAnalyze_API:app", host="0.0.0.0", port=5000)            raise ValueError(f"Role must be one of {allowed}")
        return v
